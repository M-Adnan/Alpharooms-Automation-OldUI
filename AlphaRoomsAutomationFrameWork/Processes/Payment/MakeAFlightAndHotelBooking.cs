...
{
    public class MakeAFlightAndHotelBooking : IMakeAFlightAndHotelBookingGuestDetails, IMakeAFlightAndHotelBookingGuestDetailsAuto, IMakeAFlightAndHotelBookingAllGuests
        , IMakeAFlightAndHotelBookingGuestRoom
        ...
        , IMakeABookingStaffDetails, IMakeABookingStaffDetailsAuto
    {
        ...
        public MakeAFlightAndHotelBooking(int guest)
        {
            this.guests = new List<Guest>();
            this.guests.Add(new Guest(guest));
        }

        public MakeAFlightAndHotelBooking()
        {
            this.guests = new List<Guest>();
        }
        ...
        private int GetRoomNumber(int p)
        {
            int roomIndex = 0;
            int guestCount = p;
            while (guestCount > HomePage.Data.Rooms[roomIndex].Adults.Value + HomePage.Data.Rooms[roomIndex].Children.Value)
            {
                guestCount -= HomePage.Data.Rooms[roomIndex].Adults.Value + HomePage.Data.Rooms[roomIndex].Children.Value;
                roomIndex++;
            }
            return roomIndex + 1;
        }

        private int GetGuestAge(int p)
        {
            int roomIndex = 0;
            int guestCount = p;
            while (guestCount > HomePage.Data.Rooms[roomIndex].Adults.Value + HomePage.Data.Rooms[roomIndex].Children.Value)
            ...
            return (guestCount - HomePage.Data.Rooms[roomIndex].Adults <= 0 ? 25 : HomePage.Data.Rooms[roomIndex].ChildrenAges[guestCount - 1 - HomePage.Data.Rooms[roomIndex].Adults.Value]);
        }
        ...
        public IMakeAFlightAndHotelBookingGuestDetails OfRoomNo(int room)
        {
            this.guests.Last().GuestNumber = this.guests.Count(i => i.RoomNumber == room) + 1;
            this.guests.Last().RoomNumber = room;
            return this;
        }

        public IMakeAFlightAndHotelBookingGuestDetails WithTitle(Title title)
        {
            this.guests.Last().Title = title;
            return this;
        }

        public IMakeAFlightAndHotelBookingGuestDetails WithFirstName(string firstName)
        {
            this.guests.Last().FirstName = firstName;
            return this;
        }

        public IMakeAFlightAndHotelBookingGuestDetails WithLastName(string lastName)
        {
            this.guests.Last().LastName = lastName;
            return this;
        }

        public IMakeAFlightAndHotelBookingGuestDetails WithDoB(string doB)
        {
        ...
        private void ConfirmProcess()
        {
            foreach (Guest guest in this.guests)
            {
                ...
                if (guest.DoB != null) PaymentPage.SelectGuestDoB(guest.RoomNumber, guest.GuestNumber, guest.DoB);
            }
